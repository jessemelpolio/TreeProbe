...
from models.modules import DynamicModule
...
class BaseEngine(object):
    ...
    def configure_learnable_params(
        ...
        if isinstance(param_keys, list):
            for name in param_keys:
                ...
                #     if isinstance(module, torch.Tensor) and requires_grad:
                ...
                #     for module_name, param in self.model.named_parameters():
                #         if name in module_name:
                #             print(f"Learnable parameter: {module_name}")
                #             param.requires_grad = requires_grad
                #             params.append(param)
    ...
    def evaluate_single_dataset(
        ...
        if tag == "mixed_dataset":
            breakpoints = test_dataset.breakpoints
            # breakpoints = [0, 100, 201, 397, 499]
            individual_correct = [0] * (len(test_dataset.breakpoints) - 1)
            individual_total = [0] * (len(test_dataset.breakpoints) - 1)

        for inputs, targets, text_targets in tqdm(self.test_loader):
            inputs, targets, text_targets = self.transform_data(
                inputs, targets, text_targets=text_targets
            )
            ...
            if tag == "mixed_dataset":
                for idx in range(len(breakpoints) - 1):
                    individual_indices = torch.argwhere(
                        (targets >= breakpoints[idx]) & (targets < breakpoints[idx + 1])
                    )
                    individual_total[idx] += len(individual_indices)
                    individual_correct[idx] += (
                        predicted[individual_indices]
                        .eq(targets[individual_indices])
                        .sum()
                        .item()
                    )
        ...
        if tag == "mixed_dataset":
            if self.logger is not None:
                individual_datasets = self.args.mixed_datasets.split(",")
                for idx in range(len(breakpoints) - 1):
                    self.logger.add_scalar(
                        f"test/stage_{stage}/{tag}_{individual_datasets[idx]}_acc",
                        100.0 * individual_correct[idx] / individual_total[idx],
                        epoch,
                    )

            acc_str_list = [
                "{:.1f}".format(100.0 * correct / total)
                for correct, total in zip(individual_correct, individual_total)
            ]
            acc_str = "/".join(acc_str_list)
            print("Individual Acc: ", acc_str)
            # print("Individual Acc: ", '{:.1f}'.format([100. * correct / total for correct, total in zip(individual_correct, individual_total)]))
